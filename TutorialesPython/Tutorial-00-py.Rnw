...
library(knitr)
raptor(filename = "Tutorial-00-py.Rnw", language = "python", fileOverwrite = TRUE, defaultExecOrder = Inf)
@
...
\begin{itemize}
  ...
  \item \ninja{Acude a tu ninja informático}. En particular, si tu profesor te ha dicho que leas este tutorial en algún curso universitario, pídele que te ayude con la instalación.
\end{itemize}
...
\end{itemize}
% Pero nuestro trabajo se va a concentrar en la aplicación que aparece con el nombre {\em Jupyter QTConsole}. Abre esa aplicación y verás una ventana como la que aparece en la Figura\begin{figure}[p]
...
En los próximos tutoriales tendremos ocasión de conocer a fondo el programa.  Por el momento nos vamos a tener que conformar con una brevísima introducción. El nombre Spyder es un acrónimo de {\sf {\bf S}cientific {\bf PY}thon {\bf D}evelopment {\bf E}nvi{\bf R}onment.} Es decir, un {\em entorno de desarrollo integrado} para Python con una orientación científica. En la página web 
\begin{center}
