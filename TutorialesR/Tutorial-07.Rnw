...
@
El resultado es $\Sexpr{Estadistico}$, como hemos visto en el libro.
...
@
Recuerda que el p-valor indica cómo de improbable le parecen estos datos muestrales a alguien que cree que la hipótesis nula es cierta. En este caso, a alguien que cree que el peso medio de las latas es de $1000$ gramos o más. El resultado que hemos obtenido significa que, si lo que dice el fabricante es cierto, la probabilidad de obtener al azar un lote de $100$ latas con un peso medio $\bar X = 998.5$ es aproximadamente igual a $\Sexpr{signif(pValor,4)}$. Parece bastante evidente que el inspector tendría buenas razones para poner bajo sospecha esa afirmación del fabricante.

Naturalmente, el fabricante puede insistir en que ha tenido mala suerte y que los resultados que hemos obtenido pueden ser fruto del azar... Para evitar una discusión improductiva, usamos los niveles de verosimilitud como una forma de zanjar este asunto. Podemos establecer, en los reglamentos de consumo, que los inspectores utilizarán un nivel de significación del contraste 99\%. Es decir $ns = 0.99$, con lo que $\alpha = 0.01$. Y puesto que el p-valor $\Sexpr{signif(pValor,4)}$ es  menor que $\alpha$, el inspector puede rechazar la hipótesis nula y sancionar al fabricante por faltar a la verdad sobre el peso de esas latas.
...
qnorm(alfa)
@
Ese valor del estadístico marca la frontera entre los valores que nos llevan a rechazar la hipótesis nula y los valores que no nos hacen rechazarla (recuerda que {\em nunca la aceptamos}).  Cualquier valor del estadístico menor que $\Sexpr{signif(qnorm(alfa), 4)}$ nos llevaría a rechazar $H_0$ ¿Por qué menor? Si no ves claro por qué, vuelve a leer este ejemplo y la Sección \ref{curso-cap07:sec:ContrastesUnilateralesBilaterales} del libro hasta que lo entiendas.

\begin{ejercicio}
\label{tut07:ejercicio05}
El valor $\Sexpr{signif(qnorm(alfa), 4)}$ que hemos obtenido es un valor tipificado, en la escala $Z$ de la normal estándar.  ¿Cuál es el valor correspondiente en la escala original del problema? Es decir, ¿cuál es peso medio mínimo muestral, en gramos, a partir del cual el inspector rechaza $H_0$? Solución en la página \pageref{tut07:ejercicio05:sol}.
\qed
\end{ejercicio}
...
(EstadisticoFabricante = abs(Xbar - mu0) / (s / sqrt(n)))
(pValorFabricante = 2 * (1 - pnorm(EstadisticoFabricante)))

(EstadisticoInspector = (Xbar - mu0) / (s / sqrt(n)))
(pValorInspector = (pnorm(EstadisticoInspector)))
@
...
<<t06ch08, echo=FALSE, results='hide'>>=
library(MASS)
set.seed(2014)
muestra = c(mvrnorm(100, mu = 2.65, Sigma = 0.5^2, empirical = TRUE))

mean(muestra)
sd(muestra)

write.table(muestra, file = "../datos/Tut07-Contraste-Media-Z-datos.csv",
...
<<t06ch09>>=
datos = read.table(file = "../datos/Tut07-Contraste-Media-Z-datos.csv")[ ,1]
(n = length(datos))
(Xbar = mean(datos))
(s = sd(datos))
@
...
<<tut07ej08CocinaDatos, echo=FALSE, results='hide'>>=
library(MASS)
set.seed(2014)
muestra = c(mvrnorm(300, mu = 27.1, Sigma = 0.8^2, empirical = TRUE))

mean(muestra)
sd(muestra)

write.table(muestra, file = "../datos/Tut07-Ejercicio-ContrasteMedia.csv",
        row.names = FALSE, col.names=FALSE )

t.test(muestra, alternative = "greater", mu = 27, conf.level = 0.95)
@
El fichero
\begin{center}
...
\begin{enumerate}
  ...
  \item Un laboratorio farmacéutico prepara comprimidos que deben contener una dosis de 500mg de cierto principio activo. El sistema de control de calidad del laboratorio ha tomado una muestra de 15 comprimidos para comprobar si la dosis se ajusta a lo esperado. Los valores medidos, en miligramos, son:
        ...
        library(MASS)
        set.seed(2014)
        valores = c(491, 503, 492, 502, 490, 500, 500, 501, 501, 501, 505, 491, 501, 493, 492)
        cat(paste(valores, sep="", collapse=",\\,\\,\\, "))
        @
        \]
...
library(MASS)
set.seed(2014)
muestra = as.matrix(mvrnorm(n = 125, mu = 12, Sigma = 16.85, empirical = TRUE))
write.table(muestra, file = "../datos/Tut07-Ejercicio-ContrasteVarianza.csv",
            row.names = FALSE, col.names = FALSE)
@
    ...
    \begin{itemize}
      ...
      \item Contrastes para la varianza o desviación típica  en poblaciones          normales.
                ...
                \begin{itemize}\item[]\begin{itemize}
                  ...
                  \item Estadísticos de la muestra:
                    \fichero{./code/Tut07-Contraste-Varianza.R}{Tut07-Contraste-Varianza.R}

                  \item Datos en bruto:
                    \fichero{./code/Tut07-Contraste-Varianza-DatosEnBruto.R}{Tut07-Contraste-Varianza-DatosEnBruto.R}
...
mu0=8
(contraste = t.test(datos,mu=mu0,alternative="greater",conf.level = 0.95))
@
...
muestra = read.table(file="../datos/Tut07-Contraste-Media-UsandoZ-datos.csv",sep=" ",dec=".")[,1]
mean(muestra)
(contraste = z.test(muestra, mu = 27, stdev = 1,
  alternative="greater", conf.level = 0.95))
@

Puedes ver que el p-valor (que es aproximadamente \Sexpr{contraste$p.value}) permite rechazar $H_0$, pero po rmuy poco margen. Como ves, la llamada a la función {\tt z.test} incluye el argumento {\tt stdev=1}, que representa el valor de $\sigma$, la desviación típica de la población {\bf que en este caso se supone conocida} (ya sabes que eso sucede muy pocas veces en la práctica). El argumento {\tt mu=7} se usa para indicarle a R el valor que nosotros llamamos $\mu_0$ en los contrastes. Enseguida volveremos con la segunda parte de este ejercicio, en la que se supone que $\sigma$ es desconocida. Pero antes, vamos a hacernos algunas preguntas sobre este primer cálculo, en forma de ejercicios.

\begin{ejercicio}
\label{tut07:ejercicio13}
\begin{enumerate}
  \item[]
  \item {\em En la salida de {\tt z.test} para este ejemplo se incluye un {\em intervalo de confianza unilateral}, que es $(\Sexpr{contraste$conf.int[1]}, \Sexpr{contraste$conf.int[2]})$. ¿Qué relación hay entre este intervalo y la región de rechazo para este tipo de contrastes, que aparece en la Ecuación \ref{curso-cap07:ecu:RegionRechazoMediaZColaDerecha} (pág. \pageref{curso-cap07:ecu:RegionRechazoMediaZColaDerecha}) del curso? }
...
<<>>=
z.test(muestra, mu = 27, sd = sd(muestra),
  alternative = "greater", conf.level = 0.95)
@
...
<<>>=
muestra = read.table(file="../datos/Tut07-Contraste-Varianza-datos.csv",sep=" ",dec=".")[ ,1]
...
%% Media con muestra grande.
  ...
  muPob = 12
  sigma0 = 2.5
  muestra01 = as.matrix(mvrnorm(n, mu = muPob, Sigma = sigma0^2, empirical = TRUE))
  mean(muestra01)
  fichero = "Tut07-EjAdicionales-Contrastes01.csv"
  write.table(muestra01, file = paste0("../datos/", fichero, collapse = ""),
              row.names = FALSE, col.names = FALSE)
  library(TeachingDemos)
  mu0 = muPob + 0.5

  z.test(as.vector(muestra01), mu = mu0, stdev = sigma0, alternative = "two.sided")
  @
  Fichero \fichero{../datos/\Sexpr{fichero}}{\Sexpr{fichero}}\\
...
%% Media con muestra pequeña.
  ...
  sigma0 = 1.3
  nc = 0.95
  muestra02 = as.matrix(mvrnorm(n, mu = muPob, Sigma = sigma0^2, empirical = TRUE))
  mean(muestra02)
  fichero = "Tut07-EjAdicionales-Contrastes02.csv"
  write.table(muestra02, file = paste0("../datos/", fichero, collapse = ""),
              row.names = FALSE, col.names = FALSE)
  library(TeachingDemos)
  mu0 = muPob + 0.6

  t.test(as.vector(muestra02), mu = mu0, alternative = "less", conf.level = nc)
  @
  Fichero \fichero{../datos/\Sexpr{fichero}}{\Sexpr{fichero}}\\
  Hipótesis nula: $H_0=\{\mu < \Sexpr{mu0}\}$.
  Nivel de significación: $\Sexpr{100*nc}\%$ y también $90\%$.
...
%% Varianza.
  ...
  sigma2Muestra = 1.3
  nc = 0.95
  muestra03 = as.matrix(mvrnorm(n, mu = muPob, Sigma = sigma2Muestra, empirical = TRUE))
  fichero = "Tut07-EjAdicionales-Contrastes03.csv"
  write.table(muestra03, file = paste0("../datos/", fichero, collapse = ""),
              row.names = FALSE, col.names = FALSE)
  library(TeachingDemos)
  sigma2CH = sigma2Muestra * 1.5
  sigma.test(as.vector(muestra03), sigmasq = sigma2CH, alternative = "two.sided", conf.level = nc)
  @
  Fichero \fichero{../datos/\Sexpr{fichero}}{\Sexpr{fichero}}\\
  Hipótesis nula: $H_0=\{\sigma^2 \neq \Sexpr{sigma2CH}\}$.
  Nivel de significación: $\Sexpr{100*nc}\%$.
...
\begin{enumerate}
  ...
  mu0 +  zAlfa * s /sqrt(n)
  @
  En consecuencia rechazaremos $H_0$ con cualquier valor de $\bar X$ mayor que \Sexpr{mu0 + qnorm(1 - 0.05) * s /sqrt(n)}.
...
<<t07ej08sol>>=
datos = read.table(file = "../datos/Tut07-Ejercicio-ContrasteMedia.csv")[ ,1]
(n = length(datos))
(Xbar = mean(datos))
(s = sd(datos))
mu0 = 27
...
\begin{enumerate}
  ...
  <<t07ej10sol01a>>=
  muestra = c(0.316, 0.295, 0.304, 0.263, 0.25)
  (n = length(muestra))
  (barX = mean(muestra))
  (s = sd(muestra))
  mu0 = 0.29
  ...
  \item Que la dosis sea la deseada significa que no sea ni mucho ni poco. Eso nos lleva a plantear una hipótesis nula bilateral:
      ...
      Introducimos los valores de la muestra y hacemos el cálculo del estadístico (¡bilateral, recuerda!)
        <<t07ej10sol02a>>=
          muestra = c(491, 503, 492, 502, 490, 500, 500, 501,
                501, 501, 505, 491, 501, 493, 492)
          (n = length(muestra))
          (barX = mean(muestra))
          (s = sd(muestra))
          mu0 = 500
...
<<t07ej10sol>>=
datos = read.table(file = "../datos/Tut07-Ejercicio-ContrasteVarianza.csv")[,1]
(n = length(datos))
(Xbar = mean(datos)) # Este dato no se usa en el contraste!!
(s = sd(datos))
sigma0 = 3.7
(Y = (n-1) * s^2 / sigma0^2)
k = n - 1
(pvalor = 2*(1 - pchisq(Y, df = k)))
@
