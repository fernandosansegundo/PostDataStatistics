...
<<tut09-ejemploContrasteCocienteVarianzas01, echo=FALSE, results='hide'>>=
 # PRIMERA MUESTRA
 # Numero de elementos
 (n1 = 59)
 # Cuasidesviacion tipica muestral
...
<<echo=FALSE, results='hide'>>=
scores = c(Statisti[, 1], Statisti[noAusentes, 2])
group = rep(1:2, c(length(Statisti[, 1]), length(Statisti[noAusentes, 2])) )

statisti2 = data.frame(scores=scores, group=group)
write.table(statisti2, "../datos/Tut09-Statisti2.csv")
@
...
sigma1 = 2.2
sigma2 = 9.3
muestra1 = mvrnorm(n = n1, mu = mu1, Sigma = sigma1^2, empirical = TRUE)
muestra2 = mvrnorm(n = n2, mu = mu2, Sigma = sigma2^2, empirical = TRUE)
X = c(muestra1, muestra2)
T =factor( rep(LETTERS[1:2], c(n1, n2) ))
datos = data.frame(X, T)
var.test(X ~ T, data=datos)
t.test(X ~ T, data=datos, var.equal = FALSE)
nombreFichero = "Tut09-Ejemplos-ContrasteMedias-01.csv"
write.table(datos, paste0("../datos/", nombreFichero,collapse = ""), quote = FALSE,
            sep = " ", row.names = FALSE, )
@
\begin{center}
  \fichero{../datos/\Sexpr{nombreFichero}}{\Sexpr{nombreFichero}}
\end{center}
...
datos = data.frame(X, T)
n = nrow(datos)
datos = datos[sample(1:n), ]
head(datos)
aggregate(X ~ T, data = datos, FUN = mean )
aggregate(X ~ T, data = datos, FUN = sd)
var.test(X ~ T, data=datos)
t.test(X ~ T, data=datos, var.equal = FALSE)
t.test(X ~ T, data=datos, var.equal = TRUE)


nombreFichero = "Tut09-Ejemplos-ContrasteMedias-02.csv"
write.table(datos, paste0("../datos/", nombreFichero,collapse = ""), quote = FALSE,
            sep = " ", row.names = FALSE, )
@
\begin{center}
...
<<tut09_ejercicio07_01, echo=FALSE, results='hide'>>=
  # PRIMERA MUESTRA
  # Numero de elementos
      (n1 = 286)
  # Media muestral
...
#         3 si es mu1 distinto de mu2
  ...
  #Nivel de significacion
      (nSig = 0.95)
@
<<tut09_ejercicio07_02, echo=FALSE, results='hide'>>=
  ...
  # Y ahora se aplican ambas funciones para mostrar los resultados
@
<<tut09_ejercicio07_03, echo=FALSE, results='hide'>>=
      pValor(Estadistico, TipoContraste)
      Estadistico
@
...
\begin{cases}
n_1 = \Sexpr{n1},\\
{\bar X}_1 = \Sexpr{xbar1}\\
s_1 = \Sexpr{s1}
\end{cases}
\qquad\qquad
\begin{cases}
n_2 = \Sexpr{n2},\\
{\bar X}_2 = \Sexpr{xbar2}\\
s_2 = \Sexpr{s2}
\end{cases}
\]
...
<<tut09_ejercicio08_01, echo=FALSE, results='hide'>>=
...
@
<<tut09_ejercicio08_02, echo=FALSE, results='hide'>>=
 ###############################################
 # NO CAMBIES NADA DE AQUÍ PARA ABAJO
 ###############################################
...
@
<<tut09_ejercicio08_03, echo=FALSE, results='hide'>>=
     pValor(Estadistico, TipoContraste)
     Estadistico
@
...
<<tut09_ejercicio09_01, echo=FALSE, results='hide'>>=
...
@
<<tut09_ejercicio09_02, echo=FALSE, results='hide'>>=
 ###############################################
 # NO CAMBIES NADA DE AQUÍ PARA ABAJO
 ###############################################
...
@
<<tut09_ejercicio09_03, echo=FALSE, results='hide'>>=
     pValor(Estadistico, TipoContraste)
     Estadistico
@
...
<<>>=
<<tut09_ejercicio07_01>>
@

<<echo=FALSE, results='hide'>>=
<<tut09_ejercicio07_02>>
@
Y los resultados que se obtienen son:
<<>>=
<<tut09_ejercicio07_03>>
@
...
<<echo=FALSE, results='hide'>>=
<<tut09_ejercicio08_02>>
@
Y los resultados que se obtienen son:
<<>>=
<<tut09_ejercicio08_03>>
@
...
@
<<echo=FALSE, results='hide'>>=
<<tut09_ejercicio09_02>>
@
Y los resultados que se obtienen son:
<<>>=
<<tut09_ejercicio09_03>>
@
